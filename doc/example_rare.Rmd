...
```{r read_bed}
bed_data <- read_delim(bed_file, delim = " ", col_names = c('chr', 'start', 'end', 'feature'))
# Convert to 1-based coordinates used by the VCF format, filter out unnecessary data
bed_data %<>%
...
bed_data %>% head()
```
We can see that we have a neat tibble that contains the chromosome, the start coordinate and the end coordinate of each region (row). What we need is names in the format used by VCF, i.e. `chr_no:start_coord-end_coord`. Let's generate a vector of regions in this format:
```{r construct_region_names}
regions <- paste0(str_remove(bed_data$chr, 'chr'), ':', bed_data$start, '-', bed_data$end)
regions[1:5]
```
